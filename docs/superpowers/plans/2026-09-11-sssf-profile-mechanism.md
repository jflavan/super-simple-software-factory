...
- **Caching the tree walk.** A full install traverses ~6 times: `detect_all` runs `matches()` per framework per profile, then `detect()` walks again. A per-root memo in `probes.walk` was implemented and reverted: it breaks `test_matching_requires_every_declared_framework`, which deliberately writes a package, asserts no match, then writes a `.sln` into the SAME root and asserts a match — an incremental build on one root, mid-test, which is exactly what the cache's own justification assumed did not happen. Measured cost without it, after Task 5's pruning fix: ~17ms a walk on a 25k-file repo, so ~100ms an install. Not worth an opt-in cache API. Revisit only if a much larger repo makes it visible.
