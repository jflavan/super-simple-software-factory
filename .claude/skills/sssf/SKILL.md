...
3. **Gates validate claims, not guesses** — `gate(envelope, run) -> GateReport`, one `check(item, ok, note)` per thing examined, so a green gate says *what* it verified; failures return to the same session as corrections. Every call that CHANGES the repo carries `[gates.diff_matches_claims, gates.doc_policy, *gates.profile_gates()]` — the last two are the configured contract and the generated stack gates, and leaving them off is how an ADW ends up quietly weaker than the ones shipping beside it.
...
8. **A known command is code, not an agent** — if you can write the invocation down (`bun test`, `ruff check`), it belongs in a `kind="code"` phase via `adw_modules/quality.py`. The engine is `quality.py` and never changes; the commands live in the generated `adw_modules/quality_blocks.py`. Each carries a **tier**: `fast` runs inside a bounded fix loop (`run_tests`), `full` runs once after it (`run_quality`), and asking for a tier with no blocks in it raises rather than reporting green. Agents are for the parts that need reading and deciding; failures come back to the builder as an envelope either way.
...
naming the frameworks a stack is made of. **Adding a stack is one YAML file** dropped in its own directory — `registry`
globs `*/profile.yaml`, so nothing registers it and no shared file changes.
Adding a *technology* is four: the framework module exposing eight names, its
gate module, its prompt fragment, and two lines in `frameworks/__init__.py`.
A framework never imports another framework; everything shared goes through
`probes.py` and `emit.py`. See [cookbooks/install.md](cookbooks/install.md) →
"Adding a framework".
