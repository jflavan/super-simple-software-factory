...
| `{{profile_overlay}}` | the stack fragment generated by the profile, or empty |

`{{profile_overlay}}` is the fourth, and it is the one a prompt author is most likely
not to know exists. A profiled install generates
`adws/adw_data/prompt_engineering/profile_overlay.md` — what this repo's stack is, what
its conventions are, what commands verify it — and any template naming the placeholder
receives it. A template that does not name it does not get it, and there is no warning,
so a `system.md` carried forward from an older install silently never sees the stack it
is working on. The shipped `builder/system.md` names it; diff yours against
`templates/prompt_engineering/` after an upgrade. Without a profile the substitution is
the empty string, so naming it always costs nothing.
...
│   └── quality/<seq>_<name>/command.log
│                            written by CODE, not an agent: one directory per quality
│                            block per phase, holding that command's full stdout and
│                            stderr. The block's name IS the directory name, which is
│                            why block names must be unique case-insensitively.
└── {agent_name}/
